...
import { TrendingUp, ShoppingCart, Clock, CheckCircle2, Users, Wallet } from "lucide-react";
...
function Dashboard() {
    ...
    queryFn: async () => {
      ...
      const [orders, customers, items, products, expenses, poItems] = await Promise.all([
        ...
        supabase
          ...
          .select("unit_cost, quantity, product_id, created_at")
  ...
  for (let i = 6; i >= 0; i--) {
    ...
    const total = orders
      .filter(
        (o) =>
          o.status !== "cancelled" && new Date(o.created_at) >= d && new Date(o.created_at) < next,
      )
      .reduce((s, o) => s + Number(o.total), 0);
  ...
  const monthCOGS = (data?.poItems ?? []).reduce(
    (s, p) => s + Number(p.unit_cost) * Number(p.quantity),
    0,
  );
  ...
  const bestByQty = Array.from(qtyMap.entries())
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Card>
          ...
          <CardContent className="space-y-2 text-sm">
            <div className="flex justify-between">
              ...
              <span className="font-medium">{formatKS(monthRevenue)}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-muted-foreground">- COGS (purchases)</span>
              <span>{formatKS(monthCOGS)}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-muted-foreground">- Expenses</span>
              <span>{formatKS(monthExpenses)}</span>
            </div>
            <div className="flex justify-between border-t pt-2">
              <span className="font-semibold">Net Profit</span>
              <span className={`font-bold ${monthProfit >= 0 ? "text-green-600" : "text-red-600"}`}>
                {formatKS(monthProfit)}
              </span>
            </div>
          </CardContent>
        </Card>
        <Card>
          <CardHeader>
            <CardTitle className="text-base">⚠️ Low Stock Alerts ({lowStock.length})</CardTitle>
          </CardHeader>
          <CardContent>
            {lowStock.length === 0 ? (
