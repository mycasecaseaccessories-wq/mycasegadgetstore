...
function SettingsPage() {
  ...
  useEffect(() => { if (prof) setProfile({ full_name: prof.full_name ?? "", business_name: prof.business_name ?? "" }); }, [prof]);
    ...
    <div className="grid gap-4 lg:grid-cols-2">
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          ...
          <div className="space-y-1.5"><Label>Business Name</Label><Input value={profile.business_name} onChange={e => setProfile({ ...profile, business_name: e.target.value })} /></div>
          <Button onClick={saveProfile} variant="secondary"><Save className="mr-2 h-4 w-4" />Update Profile</Button>
          <div className="space-y-1.5 border-t pt-4"><Label>Change Password</Label>
            <div className="flex gap-2"><Input type="password" placeholder="New password" value={pw} onChange={e => setPw(e.target.value)} />
              <Button onClick={changePassword}>Update</Button></div>
          </div>
        </CardContent>
      </Card>
    </div>
