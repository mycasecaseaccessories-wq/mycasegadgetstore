...
const COLORS = [
  "var(--primary)",
  "var(--accent)",
  "var(--chart-3, hsl(280 80% 60%))",
  "var(--chart-4, hsl(40 90% 55%))",
  "var(--chart-5, hsl(160 70% 45%))",
];
...
function AnalyticsPage() {
  ...
  const { data: items = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        .from("order_items")
        .select("product_name, quantity, line_total, created_at, unit_price")
        .gte("created_at", from + "T00:00:00")
        .lte("created_at", to + "T23:59:59");
      return data ?? [];
    },
  ...
  const paymentBreak = useMemo(() => {
    const m: Record<string, number> = {};
    for (const o of orders)
      if (o.status !== "cancelled") m[o.payment_status] = (m[o.payment_status] ?? 0) + 1;
    return Object.entries(m).map(([name, value]) => ({ name, value }));
  }, [orders]);
  ...
  const topProducts = useMemo(() => {
    ...
    for (const it of items) {
      ...
      c.qty += it.quantity;
      c.revenue += Number(it.line_total);
  ...
  const hourly = useMemo(() => {
    ...
    for (const o of orders)
      if (o.status !== "cancelled") arr[new Date(o.created_at).getHours()].orders++;
    return arr;
  }, [orders]);
          ...
          <CardContent>
            <ResponsiveContainer width="100%" height={240}>
              <BarChart data={hourly}>
                <CartesianGrid strokeDasharray="3 3" className="stroke-border" />
                <XAxis dataKey="hour" className="text-[10px]" interval={2} />
                <YAxis className="text-xs" />
                <Tooltip
                  contentStyle={{
                    background: "var(--popover)",
