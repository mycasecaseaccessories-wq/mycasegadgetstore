...
function InventoryPage() {
  ...
  const { data: products = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("products")
        .select(
          "id, name, image_url, price, stock_in, sold_qty, low_stock_threshold, stock_status, category",
        )
        .order("name");
  ...
  const enriched = products.map((p) => {
    ...
    const state: "ok" | "low" | "out" =
      remaining <= 0 ? "out" : remaining <= threshold ? "low" : "ok";
        ...
        <CardContent className="flex flex-wrap items-center gap-2 p-4">
          ...
          <div className="flex gap-1">
            ...
            <Button
              ...
              className={filter !== "low" ? "text-amber-600" : ""}
            >
              Low ({counts.low})
            </Button>
            <Button
              ...
              className={filter !== "out" ? "text-red-600" : ""}
            >
              Out ({counts.out})
            </Button>
          </div>
          ...
          <table className="w-full text-sm">
            <thead className="bg-muted/40 text-left text-xs uppercase text-muted-foreground">
              <tr>
                ...
                <th>Threshold</th>
                <th>State</th>
                <th className="text-right pr-4">Adjust</th>
              </tr>
            </thead>
            <tbody>
              {filtered.length === 0 && (
                  ...
                  <td className="px-4 py-2">
                    <div className="flex items-center gap-3">
                      <StorageImage
                        ...
                        fallback={<div className="h-10 w-10 rounded bg-muted" />}
                      />
