...
function ReportsPage() {
  ...
  const topProducts = useMemo(() => {
    ...
    for (const it of items) {
      ...
      cur.qty += it.quantity; cur.total += Number(it.line_total);
      map.set(it.product_name, cur);
    }
    return Array.from(map.entries()).map(([name, v]) => ({ name, ...v })).sort((a, b) => b.total - a.total).slice(0, 10);
  }, [items]);

  const exportCsv = () => {
    const rows = [
      ["Order No", "Customer", "Phone", "Status", "Payment", "Subtotal", "Discount", "Extra Fee", "Total", "Date"],
      ...orders.map(o => [o.order_no, o.customer_name ?? "", o.customer_phone ?? "", o.status, o.payment_status, o.subtotal, o.discount, o.extra_fee, o.total, o.created_at]),
    ];
    const csv = rows.map(r => r.map(v => `"${String(v).replace(/"/g, '""')}"`).join(",")).join("\n");
    const blob = new Blob([csv], { type: "text/csv" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a"); a.href = url; a.download = `sales-${from}-to-${to}.csv`; a.click();
