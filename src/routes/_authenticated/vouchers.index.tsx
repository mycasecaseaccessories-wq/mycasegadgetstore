...
function VouchersPage() {
  ...
  const filtered = vouchers.filter((v) =>
    [v.voucher_no, v.customer_name, v.customer_phone]
      .filter(Boolean)
      .join(" ")
      .toLowerCase()
    ...
    for (let i = 0; i < 3; i++) {
      ...
      if (!error && data) {
        ...
        await logActivity({
          ...
          entityType: "voucher",
          entityId: data.id,
          summary: `Voucher #${data.voucher_no}`,
        });
            ...
            <tbody>
              ...
              {filtered.map((v) => (
                <tr key={v.id} className="border-t">
                  ...
                  <td>
                    {v.payment_method ? <Badge variant="outline">{v.payment_method}</Badge> : "—"}
                  </td>
                  ...
                  <td className="text-right pr-2 whitespace-nowrap">
                    <Button asChild size="icon" variant="ghost">
                      <Link to="/vouchers/$id" params={{ id: v.id }}>
