    ...
    queryFn: async () => {
      ...
      for (const r of roles ?? []) {
        if (!map.has(r.user_id))
          map.set(r.user_id, { user_id: r.user_id, full_name: null, roles: [] });
            ...
            <form onSubmit={handleInvite} className="space-y-3">
              ...
              <div>
                ...
                <p className="mt-1 text-xs text-muted-foreground">
                  Share this with the new member; they can change it after sign-in.
                </p>
              </div>
              <div>
                <Label>Role</Label>
                ...
                <tr key={r.user_id} className="border-t">
                    ...
                    {r.roles.length === 0 ? (
                      ...
                      r.roles.map((role) => (
                        <Badge
                          key={role}
                          variant="outline"
                          className={role === "admin" ? "bg-primary/10 text-primary" : ""}
                        >
                          {role}
                        </Badge>
                      ))
                  ...
                  <td className="px-4 text-right space-x-2">
                    <Button
                      ...
                      onClick={() => toggleAdmin(r.user_id, !isAdminRow)}
                    >
