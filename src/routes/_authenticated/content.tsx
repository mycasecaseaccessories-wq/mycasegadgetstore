...
type Note = { id: string; title: string; body: string | null; pinned: boolean; created_at: string; updated_at: string };
...
function ContentPage() {
  ...
  const openEdit = (n: Note) => { setEditing(n); setTitle(n.title); setBody(n.body ?? ""); setOpen(true); };
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-3">
        ...
        <Button onClick={openNew} className="ml-auto"><Plus className="mr-2 h-4 w-4" />New Note</Button>
      </div>

      {filtered.length === 0 && (
        <Card><CardContent className="p-12 text-center text-muted-foreground">No notes yet. Click "New Note" to capture announcements, policies, or quick references.</CardContent></Card>
      )}
        ...
        {filtered.map(n => (
          <Card key={n.id} className={n.pinned ? "border-primary/50" : ""}>
            <CardContent className="space-y-2 p-4">
              ...
              {n.body && <p className="line-clamp-6 whitespace-pre-wrap text-sm text-muted-foreground">{n.body}</p>}
              ...
              <div className="flex gap-1 pt-1">
                ...
                <Button size="sm" variant="ghost" onClick={() => openEdit(n)}><Edit3 className="h-3.5 w-3.5" /></Button>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent>
          ...
          <div className="space-y-3">
            ...
            <Textarea placeholder="Write your note..." rows={8} value={body} onChange={e => setBody(e.target.value)} />
          </div>
          <DialogFooter>
