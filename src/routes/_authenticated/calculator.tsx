...
function CalculatorPage() {
  ...
  const { data: products = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("products")
        .select("id,name,price,waiting_time,size")
        .order("name");
  ...
  const addProduct = (id: string) => {
    ...
    setLines((ls) => [
      ...
      {
        product_id: p.id,
        product_name: p.name + (p.size ? ` (${p.size})` : ""),
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-3">
      <Card className="lg:col-span-2">
        <CardHeader>
          <CardTitle className="text-base">Build Order</CardTitle>
        </CardHeader>
              ...
              <SelectContent>
                {products.map((p) => (
                  <SelectItem key={p.id} value={p.id}>
                    {p.name}
                    {p.size ? ` · ${p.size}` : ""} — {formatKS(p.price)}
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <div className="space-y-1.5">
              ...
              <Input
                ...
                placeholder="Used as loyalty key"
              />
            </div>
        ...
        <CardContent className="space-y-3">
              ...
              <div className="space-y-1.5">
                ...
                <div className="flex gap-1.5">
                  ...
                  <Button
                    size="sm"
                    variant="outline"
                    onClick={() => setRedeemPts(balance)}
                    disabled={balance < cfg.minRedeem}
                  >
                    Max
                  </Button>
                </div>
                {redeemValue > 0 && (
                  <p className="text-xs text-muted-foreground">
                    −{formatKS(redeemValue)} ({safeRedeemPts} pts)
                  </p>
                )}
          ...
          {cfg.enabled && willEarn > 0 && (
            <p className="text-xs text-muted-foreground text-right">
              Will earn ~{willEarn} pts on payment
            </p>
          )}
