...
function POPage() {
  ...
  const { data: timeline = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        ...
        .select(
          "id, product_name, quantity, tracking_code, cargo_status, variant, po:purchase_orders(po_no, supplier_name, ordered_at)",
        )
  ...
  useEffect(() => {
    if (open && rate === 0 && latestRate) {
      const r =
        Number((latestRate as any).buy_rate ?? 0) + Number((latestRate as any).sell_gap ?? 0);
      if (r > 0) setRate(r);
    }
  ...
  const monthly = useMemo(() => {
    ...
    return {
      ...
      thbBought,
      thbInKs,
      grandKs: ksDirect + thbInKs,
      count: (pos as any[]).length,
      lines,
      arrived,
      pending,
      owed,
  ...
  const recordPayment = async () => {
    ...
    await supabase
      .from("purchase_orders")
      .update({ paid_amount: newPaid, payment_status: status } as any)
      .eq("id", payDialog.id);
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <Button
                      ...
                      onClick={() =>
                        setItems([
                          ...items,
                          { product_name: "", quantity: 1, cargo_status: "ordered" },
                        ])
                      }
                    >
                      + Add
                        ...
                        <div className="grid grid-cols-12 gap-2">
                          {currency === "THB" ? (
                            <Input
                              className="col-span-3"
                              type="number"
                              placeholder="THB / unit"
                              value={it.thb_price ?? ""}
                              onChange={(e) => {
                                const n = [...items];
                                n[idx] = {
                                  ...it,
                                  thb_price: e.target.value === "" ? null : Number(e.target.value),
                                };
                                setItems(n);
                              }}
                            />
                          ...
                          <Input
                            ...
                            value={it.tracking_code ?? ""}
                            onChange={(e) => {
                              const n = [...items];
                              n[idx] = { ...it, tracking_code: e.target.value };
                              setItems(n);
                            }}
                          />
                          <Select
                            ...
                            onValueChange={(v) => {
                              const n = [...items];
                              n[idx] = { ...it, cargo_status: v as CargoStatus };
                              setItems(n);
                            }}
                          >
                          ...
                          <div className="col-span-2 flex items-center justify-end text-xs text-muted-foreground">
                            {currency === "THB" && it.thb_price ? (
                              <span>≈ {formatMoney(uc)}</span>
                            ) : (
                              <span>{formatMoney(lineKs)}</span>
                            )}
                          </div>
                        </div>
                      </div>
        ...
        <Card>
          <CardContent className="p-4">
            <div className="text-xs text-muted-foreground">
              PO Count ({viewMode === "month" ? monthFilter : yearFilter})
            </div>
            <div className="mt-1 text-xl font-semibold">{monthly.count}</div>
            <div className="text-xs text-muted-foreground">
              {monthly.lines} lines · {monthly.arrived} arrived · {monthly.pending} pending
            </div>
          </CardContent>
        </Card>
        <Card>
                  ...
                  <div key={t.id} className="flex items-center justify-between gap-2 text-xs">
                    <div className="min-w-0 flex-1">
                      <div className="truncate font-medium">
                        {t.product_name}
                        {t.variant ? ` · ${t.variant}` : ""} ×{t.quantity}
                      </div>
                      <div className="truncate font-mono text-[10px] text-muted-foreground">
                        {t.tracking_code} · #{t.po?.po_no} · {t.po?.supplier_name ?? ""}
                      </div>
                    </div>
            ...
            <tbody>
              ...
              {(() => {
                    ...
                    <Fragment key={p.id}>
                      <tr className="border-t">
                        ...
                        <td className="text-right">
                          {p.currency === "THB" ? fmtTHB(Number(p.thb_total ?? 0)) : "—"}
                        </td>
                        ...
                        <td className="px-4 text-right">
                          <div className="flex justify-end gap-1">
                            ...
                            {p.status !== "received" && (
                              <Button size="sm" variant="ghost" onClick={() => receive(p.id)}>
                      ...
                      {isOpen && (
                        <tr className="border-t bg-muted/20">
                          <td colSpan={9} className="px-4 py-3">
                            <div className="space-y-1">
                              {(p.items ?? []).map((it: any) => {
                                const sell = it.product_id
                                  ? Number(productMap.get(it.product_id) ?? 0)
                                  : 0;
                                const cost = Number(it.unit_cost ?? 0);
                                const margin =
                                  sell > 0 && cost > 0 ? ((sell - cost) / cost) * 100 : null;
                                return (
                                  <div
                                    key={it.id}
                                  ...
                                  >
                                    ...
                                    <div className="col-span-2 text-muted-foreground">
                                      {it.thb_price
                                        ? `${fmtTHB(Number(it.thb_price))} / unit`
                                        : `${formatMoney(Number(it.unit_cost))} / unit`}
                                    </div>
                                    ...
                                    <div className="col-span-1 text-right">
                                      {margin !== null && (
                                        <span
                                          className={`inline-flex items-center gap-0.5 text-[10px] ${margin > 0 ? "text-green-600" : "text-red-600"}`}
                                        >
                                    ...
                                    <div
                                      className="col-span-1 truncate font-mono text-[10px] text-muted-foreground"
                                      title={it.tracking_code ?? ""}
                                    >
                                      {it.tracking_code ?? "—"}
                                    </div>
                              ...
                              {Number(p.cargo_fee ?? 0) > 0 && (
                                <div className="pt-1 text-xs text-muted-foreground">
                                  + Cargo fee: <b>{formatMoney(Number(p.cargo_fee))}</b>
                                </div>
                              )}
                              {p.note && (
                ...
                return ordered.map(([gk, list]) => {
                  ...
                  const sumOwed = list.reduce(
                    (s, p) => s + Math.max(0, Number(p.total ?? 0) - Number(p.paid_amount ?? 0)),
                    0,
                  );
                      ...
                      {viewMode === "year" && (
                        <tr className="border-t bg-muted/40">
                          <td colSpan={9} className="px-4 py-2 text-xs font-semibold">
                            {gk} · {list.length} POs · {lines} items · KS {formatMoney(sumKs)}
                            {sumThb > 0 ? ` · ${fmtTHB(sumThb)}` : ""} · paid {formatMoney(sumPaid)}
                            {sumOwed > 0 ? ` · owe ${formatMoney(sumOwed)}` : ""}
                          </td>
                        </tr>
                      )}
                      {list.map(renderRow)}
          ...
          {payDialog && (
            <div className="space-y-3">
              <div className="text-sm text-muted-foreground">
                Total: <b>{formatMoney(payDialog.total)}</b>
              </div>
              <div className="text-sm text-muted-foreground">
                Already paid: <b>{formatMoney(payDialog.paid)}</b>
              </div>
              <div className="text-sm text-red-600">
                Outstanding: <b>{formatMoney(Math.max(0, payDialog.total - payDialog.paid))}</b>
              </div>
              <div className="space-y-1.5">
