...
function RlsTestPage() {
  ...
  const tests: Test[] = [
    ...
    },
    {
      ...
      run: async () => {
        ...
        const { error } = await supabase
          .from("user_roles")
          .insert({ user_id: fakeUser, role: "admin" });
        // Cleanup if it accidentally inserted
        if (!error)
          await supabase.from("user_roles").delete().eq("user_id", fakeUser).eq("role", "admin");
    ...
    },
    {
      ...
      op: "delete",
      expect: isAdmin ? "allow" : "deny",
      label: isAdmin ? "Admin can delete (no-op on fake id)" : "Non-admin cannot delete vouchers",
      run: async () => {
          ...
          <div className="flex items-center gap-3">
            ...
            {Object.keys(results).length > 0 && (
              <div className="flex gap-2 text-xs">
                ...
                {failed > 0 && (
                  <Badge className="bg-destructive/10 text-destructive">{failed} fail</Badge>
                )}
              </div>
            )}
          </div>
