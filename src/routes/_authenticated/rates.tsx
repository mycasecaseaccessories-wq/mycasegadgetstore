...
function RatesPage() {
  ...
  const addRate = async () => {
    ...
    const { error } = await supabase
      .from("rates")
      .insert({ date, source, buy_rate: buyRate, sell_gap: sellGap, note: note || null });
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="space-y-1.5">
              <Label>Buy Rate (KS / 1 {source})</Label>
              <Input
                type="number"
                step="0.01"
                value={buyRate}
