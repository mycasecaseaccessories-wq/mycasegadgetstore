...
function VoucherDetailPage() {
  ...
  const goBackToList = async () => {
    await Promise.all([qc.invalidateQueries({ queryKey: ["vouchers"] }), router.invalidate()]);
        ...
        <CardHeader className="border-b">
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <span className="text-sm text-muted-foreground">
              {new Date(voucher.issued_at).toLocaleString()}
            </span>
          </div>
        </CardHeader>
          ...
          <div className="overflow-x-auto rounded-lg border">
                  ...
                  <tr key={i} className="border-t">
                    <td className="px-3 py-2">
                      <Input
                        ...
                        onChange={(e) =>
                          setItems((s) =>
                            s.map((x, j) => (j === i ? { ...x, product_name: e.target.value } : x)),
                          )
                        }
                      />
                    </td>
                    <td>
                      <Input
                        ...
                        onChange={(e) =>
                          setItems((s) =>
                            s.map((x, j) => (j === i ? { ...x, unit_price: +e.target.value } : x)),
                          )
                        }
                      />
                    </td>
                    <td>
                      <Input
                        ...
                        onChange={(e) =>
                          setItems((s) =>
                            s.map((x, j) =>
                              j === i ? { ...x, quantity: Math.max(1, +e.target.value) } : x,
                            ),
                          )
                        }
                      />
            ...
            <div className="p-2 print:hidden">
              <Button
                ...
                variant="outline"
                onClick={() =>
                  setItems((s) => [...s, { product_name: "", unit_price: 0, quantity: 1 }])
                }
              >
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div className="rounded-lg border bg-muted/30 p-4 space-y-1.5 text-sm">
              ...
              <div className="border-t pt-2 mt-1">
                <Row label="Total" value={formatKS(total)} bold />
              </div>
              <Row label="Paid" value={formatKS(paid)} />
              <Row label={due > 0 ? "Due" : "Change"} value={formatKS(Math.abs(due))} bold />
              {note && (
                <p className="border-t pt-2 mt-2 text-xs text-muted-foreground italic">{note}</p>
              )}
            </div>
          </div>
        </CardContent>
