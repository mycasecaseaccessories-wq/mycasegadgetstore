...
function OrdersPage() {
  ...
  const filtered = orders.filter((o) => {
    const matchesSearch =
      !search ||
      [o.customer_name, o.customer_phone, String(o.order_no)]
        .filter(Boolean)
        .join(" ")
        .toLowerCase()
  ...
  const remove = async (id: string) => {
    ...
    await logActivity({
      ...
      entityType: "order",
      entityId: id,
      summary: `Deleted order #${target?.order_no ?? id}`,
    });
    ...
    <div className="space-y-4">
              ...
              {filtered.map((o) => (
                <tr key={o.id} className="border-t">
                  ...
                  <td className="text-xs">
                    {Number(o.points_earned ?? 0) > 0 && (
                      <span className="text-emerald-600">+{o.points_earned}</span>
                    )}
                    {Number(o.points_earned ?? 0) > 0 &&
                      Number(o.points_redeemed ?? 0) > 0 &&
                      " / "}
                    {Number(o.points_redeemed ?? 0) > 0 && (
                      <span className="text-amber-600">-{o.points_redeemed}</span>
                    )}
                    {!Number(o.points_earned ?? 0) && !Number(o.points_redeemed ?? 0) && (
      ...
      <Dialog open={!!editing} onOpenChange={(v) => !v && setEditing(null)}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle>Edit Order #{editing?.order_no}</DialogTitle>
          </DialogHeader>
          {editing && (
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <div className="space-y-1.5">
                ...
                <Select
                  ...
                  onValueChange={(v) => setEditing({ ...editing, payment_status: v })}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
              ...
              <div className="sm:col-span-2 space-y-1.5">
                ...
                <Textarea
                  value={editing.delivery_note ?? ""}
                  onChange={(e) => setEditing({ ...editing, delivery_note: e.target.value })}
                />
              </div>
              <div className="sm:col-span-2 rounded-md bg-muted p-3 text-sm">
                Total: <span className="font-semibold">{formatKS(editing.total)}</span>
              </div>
            </div>
          )}
          <DialogFooter>
      ...
      <Dialog open={!!viewing} onOpenChange={(v) => !v && setViewing(null)}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle>Order #{viewing?.order_no}</DialogTitle>
          </DialogHeader>
          {viewing && (
            <div className="space-y-3 text-sm">
              <div className="grid grid-cols-2 gap-2">
                ...
                <div>
                  <span className="text-muted-foreground">Status:</span> {viewing.status}
                </div>
                <div>
                  <span className="text-muted-foreground">Payment:</span> {viewing.payment_status}
                </div>
              </div>
              ...
              <div className="space-y-1 border-t pt-2">
                <div className="flex justify-between">
                  ...
                  <span>{formatKS(viewing.subtotal)}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Discount</span>
                  <span>-{formatKS(viewing.discount)}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Extra fee</span>
                  <span>+{formatKS(viewing.extra_fee)}</span>
                </div>
                {Number(viewing.points_redeemed ?? 0) > 0 && (
                  <div className="flex justify-between text-amber-600">
                    <span>Points redeemed ({viewing.points_redeemed} pts)</span>
                    <span>-{formatKS(viewing.points_value)}</span>
                  </div>
                )}
                <div className="flex justify-between font-semibold">
                  <span>Total</span>
                  <span>{formatKS(viewing.total)}</span>
                </div>
                {Number(viewing.points_earned ?? 0) > 0 && (
                  <div className="flex justify-between text-emerald-600 text-xs pt-1">
                    <span>Points earned</span>
                    <span>+{viewing.points_earned} pts</span>
                  </div>
                )}
              </div>
