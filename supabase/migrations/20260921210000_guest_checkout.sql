...
-- Guest orders keep user_id NULL; loyalty redemption remains authenticated-only.
...
BEGIN
  ...
  END IF;
  IF v_user_id IS NULL AND v_redeem_points > 0 THEN
    RAISE EXCEPTION 'Sign in is required to redeem loyalty points';
  END IF;
    ...
    IF v_variant_id IS NOT NULL THEN
      ...
      ELSE
        v_available := COALESCE(v_variant.stock_in, 0)
          - COALESCE(v_variant.sold_qty, 0) - COALESCE(v_variant.reserved_qty, 0);
        IF v_available < v_quantity THEN RAISE EXCEPTION 'Insufficient stock for selected variant'; END IF;
    ...
    ELSE
      ...
      ELSE
        v_available := COALESCE(v_product.stock_in, 0)
          - COALESCE(v_product.sold_qty, 0) - COALESCE(v_product.reserved_qty, 0);
...
REVOKE ALL ON FUNCTION public.create_order_with_items(text, text, text, numeric, numeric, jsonb, integer) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.create_order_with_items(text, text, text, numeric, numeric, jsonb, integer) TO anon, authenticated;

-- Guest orders may save a selected payment method too. The UUID is unguessable,
-- and the phone check prevents unrelated orders from being updated.
...
BEGIN
  ...
  END IF;
  SELECT customer_phone INTO v_phone FROM public.orders
    WHERE id = p_order_id AND (user_id = v_uid OR (v_uid IS NULL AND user_id IS NULL));
  IF NOT FOUND THEN RAISE EXCEPTION 'Order not found'; END IF;
  SELECT COALESCE(SUM(deposit_required), 0) INTO v_deposit_total
...
REVOKE ALL ON FUNCTION public.finalize_order_payment_details(uuid, uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.finalize_order_payment_details(uuid, uuid) TO anon, authenticated;
